...
const quotes = [
	...
	"sooooo awesomeeee please fill these in"
]
...
func _ready() -> void:
	for i in range(20):
		...
		#comment.text = quotes[rng.randi_range(0, len(quotes)-1)]
...
func _process(delta: float) -> void:
	for comment in danmu:
		comment.position.x += comment.get_meta("speed")*delta
		comment.position.x = fmod(comment.position.x, 1920-comment.size.x)
